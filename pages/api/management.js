// Generate management tip based on lesson plan
  ...
  // if (req.method === "GET") {
    try {
      ...
      if (!data || data.length === 0) {
        ...
        // Save the generated management tip to Supabase
        const { data: updatedData, error: updateError } = await supabaseClient
          .from("lessonplans")
          .update({ management: cleanedText })
          .match({ id: id }).select();
        console.log('updatedData ', updatedData);
        if (updateError) {
          console.error("Error updating management tip in the lesson plan:", updateError);
      ...
      console.error("Error generating management tip:", error);
